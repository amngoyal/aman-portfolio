...
const StrokeText: React.FC<StrokeTextProps> = ({
  ...
  const viewBox = box ? `${box.x} ${box.y} ${box.width} ${box.height}` : `0 ${-fontSize} 1000 ${fontSize * 1.3}`;

  return (
    <span
      ref={rootRef}
      className={`stroke-text block w-full leading-none ${trigger === 'hover' ? 'cursor-pointer' : ''} ${className}`.trim()}
      style={{ 
        ...style, 
    ...
    >
      <noscript>
        <style>{`
          .stroke-text { opacity: 1 !important; }
          .stroke-text__fill { opacity: 1 !important; }
          .stroke-text__stroke { stroke-dashoffset: 0 !important; stroke-dasharray: none !important; }
        `}</style>
      </noscript>
      <svg className="stroke-text__svg block w-full h-[var(--stroke-text-height,160px)]" viewBox={viewBox} preserveAspectRatio="xMidYMid meet" aria-hidden="true">
        {fillMode === 'wipe' && box && (
          <defs>
        ...
        <text
          ref={strokeTextRef}
          className="stroke-text__stroke select-none"
          x="50%"
          y="0"
          textAnchor="middle"
          fill="none"
        ...
        <text
          className="stroke-text__fill select-none"
          x="50%"
          y="0"
          textAnchor="middle"
          fill={fillColor}
